...
import {
  APP_SETTING_ANTHROPIC_API_KEY,
  APP_SETTING_PRICE_ASSISTANT_VISIBLE,
...
export async function GET() {
  ...
  const showPriceAssistantChat = await getShowPriceAssistantChat();

  return NextResponse.json({ anthropicKeySource, showPriceAssistantChat });
}
...
export async function PUT(request: NextRequest) {
  ...
  const b = body as {
    ...
    showPriceAssistantChat?: unknown;
  };

  if (process.env.ANTHROPIC_API_KEY?.trim() && "anthropicApiKey" in b) {
  ...
  if (typeof b.showPriceAssistantChat === "boolean") {
    await prisma.appSetting.upsert({
      where: { key: APP_SETTING_PRICE_ASSISTANT_VISIBLE },
      create: {
        key: APP_SETTING_PRICE_ASSISTANT_VISIBLE,
        value: b.showPriceAssistantChat ? "true" : "false",
      },
      update: { value: b.showPriceAssistantChat ? "true" : "false" },
    });
  }

  if (!("anthropicApiKey" in b)) {
    const showPriceAssistantChat = await getShowPriceAssistantChat();
    const hasEnv = Boolean(process.env.ANTHROPIC_API_KEY?.trim());
    const dbRow = await prisma.appSetting.findUnique({
      where: { key: APP_SETTING_ANTHROPIC_API_KEY },
    ...
    return NextResponse.json({
      ok: true,
      anthropicKeySource,
      showPriceAssistantChat,
    });
  }

  const raw = b.anthropicApiKey;
  ...
  if (!key) {
    ...
    return NextResponse.json({
      ok: true,
      anthropicKeySource: "none" as const,
      showPriceAssistantChat,
    });
  }
  ...
  return NextResponse.json({
    ok: true,
    anthropicKeySource: "database" as const,
    showPriceAssistantChat,
  });
}
